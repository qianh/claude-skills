---
name: slide-deck
description: 生成针对 Nano Banana Pro 优化的专业幻灯片大纲和视觉提示词。它将你的内容转化为带有即用型设计线索的结构化叙事,让你能够即时生成高质量的幻灯片图像。输出结果组织灵活,便于在渲染最终幻灯片之前微调提示词或调整文本。适用于用户请求"创建幻灯片"、"制作PPT大纲"、"生成演示文稿"时使用。
author: 宝玉 X：@dotey 微博：@宝玉xp
version: 1.0
---
...
2. 首先生成风格指令块
3. 根据源素材和风格指令创建幻灯片大纲
4. 为每张幻灯片提供完整的四部分描述(叙事目标、关键内容、视觉画面、布局结构)
5. 确保首尾幻灯片具有特殊的视觉设计
6. 输出完整的、可直接用于设计师制作的大纲
